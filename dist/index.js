...
import * as path from "path"; // mongoose is the package that we need it to deal with mongodb database through nodejs
...
import helmet from "helmet"; // these routes are needed so we can forward our routing to separe files ( mainly for the arrangement of the code)
...
import authRouter from "../routes/Auth"; // firstly we start using our libraries which we needed and add them to our app
...
app.use(helmet()); // set the view engine to ejs
...
app.use(express.static("front")); // set mongoose connection
...
app.use(`/api/login`, authRouter); // this is the first step of initializing the server while we make the application is listening to specific port
